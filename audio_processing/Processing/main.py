...
def paso_entrenar(descriptors_dir, models_dir, n_neighbors):
    ...
    print("\n[5/5] knn_essentia")
    json_essentia = os.path.join(descriptors_dir, "music_all.json")
    if os.path.exists(json_essentia):
        try:
            n, m, c = cargar_descriptores(json_essentia)
            construir_y_guardar_knn(n, m, c, *rutas("essentia"), n_neighbors)
            modos_ok.append("essentia")
        except Exception as e:
            print(f"  ✗ Error en essentia: {e}")
    else:
        print("  ✗ knn_essentia omitido (music_all.json no encontrado)")
