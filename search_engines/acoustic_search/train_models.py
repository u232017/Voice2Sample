...
def main():
    ...
    # 5. Essentia (music_all.json — descriptores raw de Essentia)
    json_essentia = os.path.join(args.descriptors_dir, "music_all.json")
    if os.path.exists(json_essentia):
        print("\n[5/5] knn_essentia")
        n_e, m_e, c_e = cargar_descriptores(json_essentia)
        construir_y_guardar_knn(n_e, m_e, c_e, *rutas("essentia"), args.n_neighbors)
    else:
        print("\n[5/5] knn_essentia — omitido (music_all.json no encontrado)")
