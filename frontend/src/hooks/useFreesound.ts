  ...
  const searchExamples = useCallback(
    async (
      ...
      try {
        ...
        } else if (request.model === 'acoustic') {
          // All Acoustic Search focuses query the local dataset through the
          // backend KNN. 'general' becomes the internal 'acoustic_general'
          // marker (perceptual subset, no energy features); the backend maps
          // melodic/bpm/timbre to its melodia/ritmo/timbre KNN models.
          console.info(`Acoustic Search (${request.focus}): backend KNN over local dataset`);
          const focus =
            request.focus === 'general' ? ('acoustic_general' as const) : request.focus;
          sounds = await recommendationAPI.recommend({ ...request, focus }, audio, trim);

        } else {
          // Plain Freesound text search (no model)
          console.info('Freesound: text-based search');
          sounds = await freesoundAPI.search({ ...request, limit: 10 });
        }
