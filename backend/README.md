...
- **Dos motores de recomendación** sobre el dataset local:
  - **Essentia KNN**: búsqueda por descriptores acústicos (ritmo, melodía, timbre o general) usando los modelos entrenados en `audio_processing/Processing/models/`
...
- Selecciona el motor según el parámetro `model` (`essentia` | `clap`)
...
- **Importante**: `audio_processing/CLAP/regenerate_clap_embeddings.py` usa este mismo módulo para regenerar la base de embeddings, garantizando que dataset y consulta usan el mismo extractor
...
| `/api/recommendations` | POST | Devuelve hasta 4 sonidos similares. Form: `audio` (archivo), `model` (`essentia`\|`clap`), `focus` (`general`\|`melodic`\|`bpm`\|`timbre`), `limit`, `trim_start`, `trim_end` |
...
- Dataset en `Dataset/audio_processed/` y modelos KNN en `audio_processing/Processing/models/`
...
- **audio_processing/**: modelos KNN entrenados y herramientas CLAP
